...
async function init(){ setClock(); setInterval(setClock,30_000); adaptPolling(); setInterval(adaptPolling, 10*60_000); await loadGTFSWindow(); await Promise.allSettled([ loadWeather(),loadSaint(),loadTrafficMessages(),loadTransportData(),loadVelib() ]); scheduleTransportRefresh(); setInterval(loadTrafficMessages, 300_000); setInterval(()=>Promise.allSettled([loadVelib(),loadGTFSWindow()]), 600_000); }
